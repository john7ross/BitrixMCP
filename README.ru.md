...
| Для переноса задачи на **Scrum-доске спринта** нет ни одного подходящего вызова, и каждый очевидный кандидат проваливается, рапортуя об успехе: `STAGE_ID` в `tasks.task.update` меняет поле и пишет видимую всем запись в историю, но карточка стоит на месте; `kanban.addTask` только *кладёт* карточку, которой на доске нет, а для уже разложенной возвращает `true` и не делает ничего; `task.stages.movetask` отвечает `false`. | `b24_scrum_task_move` снимает карточку с доски и кладёт заново в нужную колонку (`kanban.deleteTask` → `kanban.addTask`) — проверено наблюдением за реальной доской, а не доверием к ответу API. Инструмент также предупреждает, что `STAGE_ID` не годится для проверки результата: он показывал `0`, пока карточка стояла в целевой колонке. |

## Установка
**Если у вас ничего не установлено** — берите портативный архив
(`dist/bitrix-mcp-portable.zip`, собирается `python scripts/build_portable.py`).
Внутри свой Python и все библиотеки: ни uv, ни pip, ни доступ к PyPI не нужны.
Распаковать и запустить «ПОДКЛЮЧИТЬ К CLAUDE.bat».
...
Каждый возвращает ненулевой код при провале проверки, поэтому их можно
выстраивать в CI. Помеченные *офлайн* портал не требуют, остальным нужен
доступный вебхук.

| Скрипт | Что доказывает | Нужно |
|---|---|---|
| `scripts/startup_check.py` | Сервер поднимается на обоих транспортах и регистрирует все инструменты | офлайн |
| `scripts/leak_check.py` | Санитайзер вычищает вебхуки/токены из вывода *и* из логов httpx | офлайн |
| `scripts/git_secret_scan.py` | Ни одного секрета в отслеживаемых файлах и во всей истории git | офлайн |
| `scripts/events_tools_check.py` | poll → ack → history → stats на засеянном хранилище | офлайн |
| `scripts/coverage_check.py` | Требование Т-1: каталог + диагностика scope покрывают всё API | портал |
| `scripts/poller_check.py` | Курсоры `b24_changes_since` двигаются и не теряют строки | портал |
| `scripts/pull_channel_check.py` | Push & Pull-канал подписывается и принимает события | портал |
| `scripts/receiver_e2e_check.py` | Приёмник исходящего вебхука от начала до конца, включая TLS | портал |
| `scripts/telegram_check.py` | DSL фильтра маршрутизирует нужные события | офлайн |
| `scripts/telegram_live_check.py` | Бот и чат действительно принимают сообщение | Telegram |
| `scripts/smoke.py` | Живая карта доступа на чтение по всем доменам | портал |
| `scripts/build_catalog.py` | Перегенерирует `data/catalog.json` из официальной доки | копия доки |

Пробы (диагностика, без вердикта pass/fail): `pull_probe.py`, `probe_listener.py`,
`tg_conn_probe.py`.
...
Нашли уязвимость? Сообщите приватно, а не публичным issue — как именно,
написано в [SECURITY.md](SECURITY.md). URL вебхука, с которым работает сервер,
это предъявительские учётные данные ко всему порталу.
